...
class Parser
{
	...
	private function doParse($value, $flags)
	{
		...
		do
		{
			...
			if (self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values))
			{
				...
				} else if (NULL !== $subTag = $this->getLineTag(ltrim($values['value'], ' '), $flags))
				{
			...
			)
			{
				...
				} else if (isset($values['value']) && self::preg_match('#^&(?P<ref>[^ ]++) *+(?P<value>.*)#u', $values['value'], $matches))
				{
					$isRef = $matches['ref'];
					$values['value'] = $matches['value'];
				}
				...
				{
					// Merge keys
				} else if (!isset($values['value']) || '' === $values['value'] || 0 === strpos($values['value'], '#') || (NULL !== $subTag = $this->getLineTag($values['value'], $flags)))
				{
					// hash
				...
				if (0 === $this->currentLineNb)
				{
					...
					foreach ($this->lines as $line)
					{
						try
						{
							...
							} else if ($previousLineWasNewline)
							{
	...
	private function parseBlockScalar($style, $chomping = '', $indentation = 0)
	{
		...
		if ($notEOF)
		{
			$blockLines[] = '';
			$this->moveToPreviousLine();
		} else if (!$notEOF && !$this->isCurrentLineLastLineInDocument())
		{
			$blockLines[] = '';
		}
